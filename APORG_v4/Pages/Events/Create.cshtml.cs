...
namespace APORG_v4.Pages.Events
{
    ...
    public class CreateModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var EventFromDb = _db.Events.Find(Event.id);
            ...
            if(files[0] != null && files[0].Length > 0)
            {
                ...
                EventFromDb.Image = @"\images\" + Event.id + extension;
            }
            else
            {
                var uploads = Path.Combine(webRootPath, @"images\" + SD.DefaultEventImage);
                System.IO.File.Copy(uploads, webRootPath + @"\images\" + Event.id + ".png");
                EventFromDb.Image = @"\images\" + Event.id + ".png";
            }
